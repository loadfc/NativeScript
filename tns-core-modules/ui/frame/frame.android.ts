...
export function reloadPage(context?: ModuleContext): void {
    ...
    if (callbacks) {
        ...
        const isRootModuleChanged = context && context.path.includes(application.getMainEntry().moduleName) && context.type !== "style";

        // Reset activity content on a change in app root module
        if (isRootModuleChanged || !rootView || !rootView._onLivesync(context)) {
