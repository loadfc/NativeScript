...
export function livesync(rootView: View, context?: ModuleContext) {
    ...
    let recreateModule = false;

    if (context && context.path) {
        const markupExtensions = ["xml"]; // html
        ...
        if (!reapplyAppStyles) {
            ...
            recreateModule = markupExtensions.some(ext => context.path.endsWith(ext));
        }
    }
    ...
    if (reapplyAppStyles && rootView) {
        ...
        reapplyLocalStyles || recreateModule ? liveSyncCore(context) : liveSyncCore();
    }
}
